...
class Connect:
    ...
    def get_next_state(self, state, action):
        clone = np.copy(self.grid) # Clone the current state of board, so it can be changed back at the end of function.
        self.grid = np.copy(state) # Make the given state the state of the board.
        self.act(action) # Take action. 
        next_state = np.copy(self.grid) # Get new_state.
        self.grid = np.copy(clone) # Return the grid back to what it was.
        return next_state
